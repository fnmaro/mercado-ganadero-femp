...
function addLineage(dato, fuente, url, estado, confianza) {
  const entry = {
    ...
    estado, confianza,
    hash: crypto.createHash('sha256').update(dato + fuente + estado + Date.now()).digest('hex').substring(0, 16)
  };
...
async function fetchHTML(url, opts = {}) {
  for (let i = 0; i < CONFIG.maxRetries; i++) {
    try {
      ...
      return { success: true, html, checksum: checksum(html) };
    } catch (e) {
      if (i === CONFIG.maxRetries - 1) return { success: false, error: e.message };
      await new Promise(r => setTimeout(r, 1000 * (i + 1)));
    }
...
async function scrapeWithPuppeteer(url, waitForSelector, extractFn) {
  ...
  try {
    ...
    await browser.close();
    return { success: true, data: result, checksum: checksum(result) };
  } catch (e) {
