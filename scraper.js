...
async function sendTelegramAlert() {
  ...
  const mensaje = `🚨 *FEMP V4.0 - Reporte Crítico*\n📅 ${today} | ⏱️ ${time}\n\n` + 
...
function addLineage(dato, fuente, url, estado, confianza) {
  STATE.lineage.push({
    dato, fuente, url: url.substring(0, 80), timestamp: new Date().toLocaleString('es-AR'),
    estado, confianza, hash: require('crypto').createHash('sha256').update(dato + fuente + estado + Date.now()).digest('hex').substring(0, 16)
  });
}
...
async function fetchJSON(url, opts = {}) {
  for (let i = 0; i < CONFIG.maxRetries; i++) {
    try {
      ...
      const res = await fetch(url, { ...opts, signal: controller.signal, headers: { 'Accept': 'application/json', 'User-Agent': 'FEMP-Bot/4.0', ...opts.headers } });
      clearTimeout(tid);
...
async function fetchAPEA() {
  ...
  const d = res.data;
  const result = {
    ocupacion: d.ocupacion || 70, reposicion: d.reposicion || 1.30, variacion: 4, hilton: d.hilton || 24000,
    novMestizo: { min: 7900, max: 8200 }, vacaCorte: { min: 7500, max: 7800 }, novCruza: { min: 8000, max: 8300 }, vacaManuf: { min: 7200, max: 7500 },
...
async function fetchTradicionCeres() {
  ...
  if (res.success) {
    ...
    if (html.includes('ceres') || html.includes('tradicion')) {
      ...
      return {
        tradicion: { fecha: today, hora: '10:00', lugar: 'Ceres, Santa Fe', ternero: 6200, novillito: 5500, vaquillona160: 5950, vaquillona200: 5700, vacaGorda: 3150, novilloGordo: 4450, promMesAnterior: 4380, fuente: 'Tradicion / Infocampo' },
...
function getSeedData() {
  return {
    ...
    canuelas: { vacaGorda: { precio: 3197, unidad: '$/kg' }, novilloGordo: { precio: 4419, unidad: '$/kg' }, vaquillona270: { precio: 4921, unidad: '$/kg' } },
    rosgan: [],
    tradicion: { ternero: 6200, novillito: 5500, vacaGorda: 3150, novilloGordo: 4450 },
    ceres: { ternero: 6373, novillito: 5380, vacaGorda: 3200, novilloGordo: 4500 },
    apea: { novMestizo: { max: 8200 }, hilton: 24000 },
...
async function main() {
  ...
  if (!diaMercado) {
    if (ultimoDatos) {
      ...
      datos.diaMercado = false; datos._modo = 'SEED';
    }
  } else {
    ...
    if (tc) { datos.tradicion = tc.tradicion; datos.ceres = tc.ceres; } 
    else if (ultimoDatos) { datos.tradicion = ultimoDatos.tradicion; datos.ceres = ultimoDatos.ceres; }
  ...
  // Bases históricas (Abril 2026) para normalizar a índice 100
  const BASE_CONSUMO = 4419;
  const BASE_INVERNADA = 6373;
  const BASE_EXPO = 8200;

  const pConsumo = datos.canuelas?.novilloGordo?.precio || datos.ceres?.novilloGordo || BASE_CONSUMO;
  const pInvernada = datos.ceres?.ternero || datos.tradicion?.ternero || BASE_INVERNADA;
  const pExportacion = datos.apea?.novMestizo?.max || BASE_EXPO;

  const iConsumo = (pConsumo / BASE_CONSUMO) * 100;
  const iInvernada = (pInvernada / BASE_INVERNADA) * 100;
  const iExportacion = (pExportacion / BASE_EXPO) * 100;

  // Ponderación: 50% Consumo, 30% Invernada, 20% Exportación
  datos.ganaderoIndex = parseFloat(((iConsumo * 0.50) + (iInvernada * 0.30) + (iExportacion * 0.20)).toFixed(2));

  // Generación de SMAs y Señales
  const indices = history.map(h => h.ganaderoIndex).filter(v => v !== undefined);
  ...
  // Alerta de cambio de tendencia macro
  const lastSignal = history.length > 0 ? history[history.length - 1].senal : 'MANTENER';
  if (senal !== lastSignal && senal !== 'MANTENER' && diaMercado) {
    STATE.alertasCriticas.push(`🔄 CAMBIO DE TENDENCIA MACRO: El Ganadero Index emite señal de ${senal}. Índice actual: ${datos.ganaderoIndex}`);
  }
  ...
  datos._confianza = calcularConfianzaGlobal();
  ...
  history.push({
    ...
    ganaderoIndex: datos.ganaderoIndex,
    senal: senal
  });
  ...
  log('========================================');
  log(`Ganadero Index: ${datos.ganaderoIndex} | Señal: ${senal}`);
  log('========================================');
}
